...
nrow(d[is.na(d$loglik), ]) #40 failures
...
d <- d |> 
  mutate(
    ...
    method = factor(
      method, 
      levels = c("DNM", "EM", "EM-DNM"), 
      labels = c("L-BFGS", "EM", "EM-L-BFGS"))
  )
...
ggsave("figures/max_A_S3_em_lbfgs.pdf", width = 6.5, height = 4)
